 ...
 *
 * Helper test script that, provided a directory, parses each .dat file and compares against the same named .json
 *
...
import {parseAtcf} from './index.js';
...
(() => {
    ...
    for (let test of testFiles) {
        if (!test[1]['.dat']) {
            console.warn(`Test file ${test[0]} does not have a dat. Skipping.`);
            continue;
        }

        try {
            const testText = nodeFs.readFileSync(test[0] + '.dat');
            const atcfFile = parseAtcf(testText.toString());

            if (test[1]['.json']) {
                const jsonText = nodeFs.readFileSync(test[0] + '.json').toString();
                const parsedStr = JSON.stringify(atcfFile);
